...
def xavier_normal_small_init_(tensor:torch.Tensor, gain=1.):
...
def xavier_uniform_small_init_(tensor:torch.Tensor, gain=1.) -> torch.Tensor:
    fan_in, fan_out = _calculate_fan_in_and_fan_out(tensor)
    std = gain * math.sqrt(2.0 / float(fan_in + 4*fan_out))
...
# originally from transformer.py --------------------------------------

### Model definition

def make_model(only_generator_init=False, d_atom=28, N=2, d_model=128, h=8, dropout=0.1, 
    ...
    for name, p in model.named_parameters():
        #print(name)
        if p.dim() > 1 and ('generator' in name or not only_generator_init):
            #print('init this')
...
class GraphTransformer(nn.Module):
    ...
    def forward(self, node_features, adj_matrix, distances_matrix, edges_att=None):
        "Take in and process masked src and target sequences."
        src_mask = torch.sum(torch.abs(node_features), dim=-1) != 0
        return self.predict(self.encode(node_features, src_mask, adj_matrix, distances_matrix, edges_att), src_mask)
...
class SublayerConnection(nn.Module):
    ...
    def __init__(self, size, dropout, scale_norm, use_adapter):
        ...
        self.adapter = None #Adapter(size, 8) if use_adapter else None
...
def load_pretrained_mat(model_params, pretrained_path):
    model = make_model(only_generator_init=True, **model_params)
